...
public class Spring11JpaApplication {
	...
	public void galleryTest() {
		...
		Gallery g1=Gallery.builder().writer("김구라").title("제목").build();
		Gallery g2=Gallery.builder().writer("김구라2").title("제목2").build();
		Gallery g3=Gallery.builder().writer("김구라3").title("제목3").build();
		...
		list.stream().forEach(item->{
			// GalleryDto 에는 @Data 어노테이션이 붙어 있어서 객체에 저장된 내용을 확인할수 있다.
		...
		//PhoneRepository 객체를 이용하지 않고 EntityManagerFactory 객체를 이용해서 직접 Gallery 저장해보기
		...
		try {
			...
			// EntityManager 객체를 이용해서 Gallery 객체를 영속화(영구저장, DB 에저장) 하겠다는 의미
			em.persist(g4);
			tx.commit();
		}catch(Exception e) {
			e.printStackTrace();
